...
int main(int argc, char** argv)
{
  ...
  if(argc!=2){
	fprintf(stderr, "Usage: MPI_Reduce Size_Element \n");
	exit(1);
}	
...
//	printf("procesos: %d\n",numero_por_cada_proceso);
   ...
   // es decir vector que recibe cada procesos
    float *rec_vector = (float*) malloc(sizeof(float) * Longitud/worldsize);
...
//	printf("Proceso %d, promedio:%f\n ",myrank,sub_avg);
 ...
 //gather
   ...
   if(myrank == 0){
      ...
      //Uso de funcion reduce
       float Global_Add;
       MPI_Reduce(&sub_avg,&Global_Add, 1, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);
   
 	//root calcula los promedio llegados en el buffer del gather
	if(myrank == 0){
	printf("valor de reduce: %f\n",Global_Add);
	float promediox = Global_Add/worldsize;
//	float total_pro = compute_avg(vect_gather,worldsize);
	printf("El promedio de todos los elementos es: %f\n",promediox);
	
	printf("promedio de arreglo normal");
	float sumas= compute_avg(numero_alea,Size_Elements);
	printf(":%f\n",sumas);
